...
<<beginning, echo=F>>=
opts_chunk$set(echo=FALSE, fig.path="../figures/", cache.path='../cache/', cache=TRUE, fig.width=13.0, fig.height=9, dev="pdf")
require(xlsx)
@
...
for (i in 1:4) {
  if (i==1) cobby <- read.xlsx2(infile, sheetName=sheets[1], colIndex=c(1:3,13), header=T, stringsAsFactors=F, colClasses=c('integer', rep('numeric', 3)))
  if (i==2) berri <- read.xlsx2(infile, sheetName=sheets[2], colIndex=c(1:3,13), header=T, stringsAsFactors=F, colClasses=c('integer', rep('numeric', 3)))
  if (i==3) renmark <- read.xlsx2(infile, sheetName=sheets[3], colIndex=c(1:3,13), header=T, stringsAsFactors=F, colClasses=c('integer', rep('numeric', 3)))
  if (i==4) chaffey <- read.xlsx2(infile, sheetName=sheets[4], colIndex=c(1:3,13), header=T, stringsAsFactors=F, colClasses=c('integer', rep('numeric', 3)))
  }
...
# rename columns
names(cobby) <- names(berri) <- names(renmark) <- names(chaffey) <- c('year', 'area', 'pumped', 'accessions')
...
Accession rates were initially high at \Sexpr{round(ave1935, 1)} Ml/ha reflecting estimated high application rates of irrigation (Appirr), low irrigation efficiency (IE), and spillaage and transmission losses. As \textbf{Appirr} decline, \textbf{IE} improved, and comprehensive drainage schemes (CDS) were implemented from 1935 onwards and the delivery infrastructure was rehabilitated the accession rates declined at an average of \Sexpr{round(abs(decline$coeff[2]),3)} ML/ha/yr. From 1992 onwards, when records were available the average accession rate was \Sexpr{round(ave2000s,1)} ML/ha reflecting improving \textbf{IE} and the effects of the 2005 -- 2010 drought. Since 2010, there has been a slight increase in accession rate.  
...
\begin{figure} 
<<plotacc, echo=F, out.width='0.95\textwidth', fig=T>>=
# plot out accession rate
# applied and accessions
opar <- par(mar=c(c(5.1,7.1,2.1,6.1)))
with(renmark, plot(year, (accessions/area), type='l', lwd=2, col='red', xlab='', ylab=''))
title(main="Irrigation Accessions for Renmark, Berri, Cobdogla and Chaffey Irrigation Districts", cex.main=1.75)
title(xlab="Year", ylab="Accession rate (ML/ha)", cex.lab=1.5)
...
\section{Cobdogla}
<<Cobdogla, echo=F, out.width='0.95\textwidth', fig=T>>=
# plot year by accession volumes
opar <- par(mar=c(c(5.1,8.1,2.1,7.1)))
with(cobby, {
  plot(year, pumped, col="blue", type='l', lwd=2, xlab='', ylab='')
...
\begin{sidewaysfigure}
\section{Berri}
<<Berri, echo=F, out.width='0.95\textwidth', fig=T>>=
# plot year by accession volumes
opar <- par(mar=c(c(5.1,8.1,2.1,7.1)))
with(berri, {
  plot(year, pumped, col="blue", type='l', lwd=2, xlab='', ylab='')
...
\begin{sidewaysfigure}
\section{Renmark}
<<Renmark, echo=F, out.width='0.95\textwidth', fig=T>>=
# plot year by accession volumes
opar <- par(mar=c(c(5.1,8.1,2.1,7.1)))
with(renmark, {
  plot(year, pumped, col="blue", type='l', lwd=2, xlab='', ylab='')
...
\begin{sidewaysfigure}
\section{Chaffey}
<<Chaffey, echo=F, out.width='0.95\textwidth', fig=T>>=
# plot year by accession volumes
opar <- par(mar=c(c(5.1,8.1,2.1,7.1)))
with(chaffey, {
  plot(year, pumped, col="blue", type='l', lwd=2, xlab='', ylab='')
