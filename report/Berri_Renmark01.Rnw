...
<<cds, echo=F, out.width='0.95\\linewidth'>>=
...
with(subset(df3, year > 2003 & caisson !="5"), {
  ...
  text(21000, 200, paste('Equation of line is: ML = 0.0124 * kW'))
  arrows(20000,210, 18500, 230, length=0.1, code=2, lwd=2)
...
<<readacc, echo=FALSE>>=
infile <- "C:\\Users\\Tony\\Laroona Consulting\\IIM\\Berri_Renmark\\data\\Irrigation Accessions.xlsx"
sheets <- vector("character", 4)
sheets[1] <- "Cobdogla"; sheets[2] <- "Berri"; sheets[3] <- "Renmark"; sheets[4] <- "Chaffey"

for (i in 1:4) {
  if (i==1) cobby <- read.xlsx2(infile, sheetName=sheets[1], colIndex=c(1:3,13,15), header=T, stringsAsFactors=F, colClasses=c('integer', rep('numeric', 4)))
  if (i==2) berri <- read.xlsx2(infile, sheetName=sheets[2], colIndex=c(1:3,13,15), header=T, stringsAsFactors=F, colClasses=c('integer', rep('numeric', 4)))
  if (i==3) renmark <- read.xlsx2(infile, sheetName=sheets[3], colIndex=c(1:3,13,15), header=T, stringsAsFactors=F, colClasses=c('integer', rep('numeric', 4)))
  if (i==4) chaffey <- read.xlsx2(infile, sheetName=sheets[4], colIndex=c(1:3,13,15), header=T, stringsAsFactors=F, colClasses=c('integer', rep('numeric', 4)))
  }

# replace NAN and 0 with NA
for (j in 2:5) {
  cobby[,j] <- ifelse(is.nan(cobby[,j]), NA, cobby[,j])
  berri[,j] <- ifelse(berri[,j]==0, NA, berri[,j])
  renmark[,j] <- ifelse(renmark[,j]==0, NA, renmark[,j])
  chaffey[,j] <- ifelse(chaffey[,j]==0, NA, chaffey[,j])
  }
  
# rename columns
names(cobby) <- names(berri) <- names(renmark) <- names(chaffey) <- c('year', 'area', 'pumped', 'accessions', 'totappld')

# calculate average accession rates for 1900-1935, rate of decline 1936-1992 and average
# for 1992-2014
alldist <- rbind(cobby, berri, renmark, chaffey)
ave1935 <- with(alldist[(alldist$year <=1935),], mean(accessions/area, na.rm=T))
avevol35 <- with(alldist[(alldist$year <=1935),], mean(totappld, na.rm=T))
ave2000s <- with(alldist[(alldist$year >=1992),], mean(accessions/area, na.rm=T))

# rate of decline
decline <- with(alldist[(alldist$year > 1935 & alldist$year <= 1992),], lm(accessions/area~year))
@
...
Accession rates were initially high at \Sexpr{round(ave1935, 1)} ML/ha -- about \Sexpr{round(ave1935/avevol35*100, 0)}\% of total applications -- reflecting estimated high application rates of irrigation, low irrigation efficiency, spillage and transmission losses. As application rates declined and irrigation efficiency improved and delivery infrastructure rehabilitated from mid 1980s onwards, the accession rates declined at an average of \Sexpr{round(abs(decline$coeff[2]),3)} ML/ha/yr. From 1992 onwards, when records were available the average accession rate was \Sexpr{round(ave2000s,1)} ML/ha reflecting improving irrigation efficiency and the effects of the 2005 -- 2010 drought. Since 2010, there has been a small increase in accession rate. This reduction in irrigation accessions is reflected in the decline in the volume of water captured by the CDS shown by the data of RIT.
...
\begin{sidewaysfigure} 
<<plotacc, echo=F >>=
# plot out accession rate
# applied and accessions
opar <- par(mar=c(c(5.1,8.1,2.1,6.1)))
with(renmark, plot(year, (accessions/area), type='l', lwd=2, col='red', xlab='', ylab='', cex.axis=1.5))
#title(main="Irrigation Accessions for Renmark, Berri, \n Cobdogla and Chaffey Irrigation Districts", cex.main=1.75)
title(xlab="Year", ylab="Accession rate (ML/ha)", cex.lab=2.0)
with(berri, lines(year, (accessions/area), lwd=2, col='brown'))
with(cobby, lines(year, (accessions/area), lwd=2, col='blue'))
with(chaffey, lines(year, (accessions/area), lwd=2, col='dark green'))

# draw legend
legend("topright", legend=c('Renmark','Berri','Cobdogla','Chaffey'), col=c('red','brown','blue','dark green'), lty=1, lwd=2)
par(opar)
@
...
<<'change', echo=F>>=
opts_chunk$set(fig.width=19, fig.height=12, dev="pdf", out.width='\\textwidth')
@
...
\section{Cobdogla}
<<Cobdogla, echo=F>>=
# plot year by accession volumes
opar <- par(mar=c(c(5.1,8.6,2.1,7.1)))
with(cobby, {
  plot(year, pumped, col="blue", type='l', lwd=2, xlab='', ylab='', cex.axis=1.5)
  title(xlab="Year", ylab="Volume (ML)", cex.lab=2)
  lines(year, accessions, col="red", lwd=2)
  legend("topleft", legend=c('Pumped Volume', 'Accessions'), col=c('blue',"red"), lty=1, lwd=2)
  })
par(opar)
@
